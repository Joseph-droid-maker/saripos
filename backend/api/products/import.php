...
// Supports existing spreadsheet format AND standard CSV format
$col_aliases = [
...
if (!isset($col['price'])) { $db->close(); respondError("Could not find a price/retail price column. Headers found: " . implode(', ', $header)); }
...
while (($row = fgetcsv($handle)) !== false) {
    ...
    // Skip blank rows (common in spreadsheets)
    ...
    if ($price <= 0) { $errors[] = "Row $rowNum ($name): invalid price '$price'."; $skipped++; continue; }
        ...
        $stmt = $db->prepare(
            'INSERT INTO products (name, description, sku, price, cost_price, stock, category_id)
             ...
             ON DUPLICATE KEY UPDATE
               ...
               stock=stock + VALUES(stock), category_id=VALUES(category_id), is_active=1'
        );
...
respond(true, [
    ...
    'skipped'  => $skipped,
    'errors'   => $errors,
], "Import complete. Added: $inserted, Updated: $updated, Skipped: $skipped.");
